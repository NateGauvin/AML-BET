...
riskByExpression <- function(dataset, gene) {
  ...
  ggplot(filterset, aes(x = factor(risk, levels = c("Favorable", "Intermediate", "Adverse")), y = expression, fill = risk)) + geom_boxplot() + 
    labs(title = ,x = "Level of Risk", y = "Expression (Log Counts per Million)") +
    ggtitle(paste("Expression of Gene", gene, "by Risk")) + theme_classic()
}

riskByExpression(beat, "ALDH1A1")
